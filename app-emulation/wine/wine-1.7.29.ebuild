...
NINE_REV="053373c"
NINE_PATCH="wine-1.7.29-d3d9-${NINE_REV}.patch"
...
SRC_URI="${SRC_URI}
	...
	pipelight? ( https://github.com/compholio/wine-compholio/archive/v${COMPHOLIOV}.tar.gz -> ${COMPHOLIO_PATCHES}.tar.gz )
	pulseaudio? ( https://github.com/compholio/wine-compholio/archive/v${COMPHOLIOV}.tar.gz -> ${COMPHOLIO_PATCHES}.tar.gz )"
...
src_unpack() {
	...
	if (use pipelight || use pulseaudio); then
		...
		mv "${COMPHOLIO_PATCHES}/patches/winepulse-PulseAudio_Support" "${T}" || die
	fi
...
src_prepare() {
	...
	use pulseaudio && PATCHES+=(
		"${T}/winepulse-PulseAudio_Support"/*.patch
	)
