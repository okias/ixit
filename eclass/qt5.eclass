...
# @BLURB: Eclass for Qt5-based packages, second edition.
# @DESCRIPTION:
# This eclass contains various functions that may be useful when
# dealing with packages using Qt5 libraries. Requires EAPI=5 or later.

case ${EAPI} in
	5) : ;;
	*)	 die "qt5.eclass: unsupported EAPI=${EAPI:-0}" ;;
esac
...
# @FUNCTION: qt5_src_unpack
# @DESCRIPTION:
# Default src_unpack function for packages that depend on qt5. If you have to
# override src_unpack in your ebuild (probably you don't need to), call
# qt5_src_unpack in it.
qt5_src_unpack() {
...
# @FUNCTION: qt5_src_prepare
# @DESCRIPTION:
# Default src_prepare function for packages that depend on qt5. If you have to
# override src_prepare in your ebuild, you should call qt5_src_prepare in it,
...
# @FUNCTION: qt5_src_configure
# @DESCRIPTION:
# Default src_configure function for packages that depend on qt5. If you have to
# override src_configure in your ebuild, call qt5_src_configure in it.
qt5_src_configure() {
	debug-print-function $FUNCNAME "$@"
...
# @FUNCTION: qt5_src_compile
# @DESCRIPTION:
# Default src_compile function for packages that depend on qt5. If you have to
# override src_compile in your ebuild (probably you don't need to), call
# qt5_src_compile in it.
qt5_src_compile() {
	debug-print-function $FUNCNAME "$@"
...
# @FUNCTION: qt5_src_install
# @DESCRIPTION:
# Default src_install function for qt5-based packages. Installs compiled code,
...
qt5_src_install() {
	...
	[[ -n ${LANGS} || -n ${LANGSLONG} ]] && qt5_install_translations
}

# @FUNCTION: eqmake5
# @USAGE: [project_file] [parameters to qmake]
# @DESCRIPTION:
# Wrapper for Qt5's qmake. If project_file isn't specified, eqmake5 will
...
# that it exists. Otherwise eqmake5 fails.
#
...
# should run eqmake5 on the top-level project file only, unless you have
...
eqmake5() {
	...
	local file=
	while read file; do
		grep -q '^### eqmake5 was here ###$' "${file}" && continue
		local retval=$({
				rm -f "${file}" || echo FAIL
				awk -v file="${file}" \
					-v add=${config_add} \
		...
		if [[ ${retval} == 1 ]]; then
			...
			die "eqmake5 failed to process ${file}"
		fi
	done < <(find . -type f -name '*.pr[io]' -printf '%P\n' 2>/dev/null)

	"${EPREFIX}"/usr/lib/qt5/bin/qmake \
		-makefile \
		...
		QMAKE_LFLAGS_DEBUG= \
		QMAKE_LIBDIR_QT="${EPREFIX}"/usr/$(get_libdir)/qt5 \
...
# Internal function, used by eqmake5 and qt5_src_configure.
# Outputs a project file name that can be passed to eqmake5. Sets nullglob
